...
describe('<Button />', () => {
  ...
  it('changes backgroundColor and hoverColor Props', () => {
    const backgrondColor = '#FF1744';
    const hoverColor = '#1E40FF';
    render(<Button label="추가" backgroundColor={backgrondColor} hoverColor={hoverColor} />);

    const parent = screen.getByText('추가').parentElement;
    expect(parent).toHaveStyleRule('background-color', backgrondColor);
